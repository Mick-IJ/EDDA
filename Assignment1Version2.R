...
for(i in 1:B) {
  Xstar=sample(telephone$Bills,replace=TRUE) 
  Tstar10[i]=mean(Xstar < 10) 
}

# test median bill <10 
pl = sum(Tstar10<0.25)/B
pr= sum(Tstar10>=0.25)/B
p = 2*(min(pl,pr))
mean(Tstar10)
...
hist(run$before-run$after)
hist(run$before, main = "qq plot before")
hist(run$after, main = "qq plot before")
...
#energydrink
qqnorm(run$before[13:24]-run$after[13:24]) #not normal-> so wilcox
wilcox.test(run$before[13:24]-run$after[13:24], mu=0) 
#Conclusion: p = 0.09983, also no difference in energy drink condition

#c)
difference = run$before - run$after #positive values mean faster times
qqnorm(difference[1:12]) #not normal -> mann whitney u
qqnorm(difference[13:24]) #not normal -> mann whitney u
wilcox.test(difference[1:12], difference[13:24])
#Conclusion: p-value = 0.1332, so the drinks do not affect difference in times
...
#plot for KS
par(mfrow=c(1,2))
hist(chickwts$weight[which(chickwts$feed=='sunflower')]) #add qqnorm
